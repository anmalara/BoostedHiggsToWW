#include "UHH2/BoostedHiggsToWW/include/GenericHists.h"
...
GenericHists::GenericHists(Context & ctx, const string & dirname): Hists(ctx, dirname){
...
void GenericHists::fill(const Event & event){
  ...
  weights->Fill(w,1);
  std::vector<TopJet>*  jets = event.JETVARIABLE;
  int Njets = (jets)->size();
