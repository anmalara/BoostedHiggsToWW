...
namespace uhh2examples {
  ...
  */
  ...
  BoostedHiggsToWWModule::BoostedHiggsToWWModule(Context & ctx){
    ...
    dijet_sel.reset(new DijetSelection()); // see BoostedHiggsToWWSelections

    // 3. Set up Hists classes:
    h_nocuts.reset(new BoostedHiggsToWWHists(ctx, "NoCuts"));
    h_njet.reset(new BoostedHiggsToWWHists(ctx, "Njet"));
    h_dijet.reset(new BoostedHiggsToWWHists(ctx, "Dijet"));
  ...
  bool BoostedHiggsToWWModule::process(Event & event) {
    ...
    cout << "BoostedHiggsToWWModule: Starting to process event (runid, eventid) = (" << event.run << ", " << event.event << "); weight = " << event.weight << endl;
