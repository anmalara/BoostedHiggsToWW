...
void HistsBase::book_TH1F(const string& name_, const int xnbins_, const double* xbin_ls_){
  if(h1.find(name_) == h1.end()) h1[name_] = book<TH1F>(name_.c_str(), name_.c_str(), xnbins_, xbin_ls_);
  else throw runtime_error("HistsBase::book_TH1F -- histogram key already existing: "+name_);
  return;
}

void HistsBase::book_TH1F(const string& name_, const int xnbins_, const float* xbin_ls_){
  if(h1.find(name_) == h1.end()) h1[name_] = book<TH1F>(name_.c_str(), name_.c_str(), xnbins_, xbin_ls_);
  else throw runtime_error("HistsBase::book_TH1F -- histogram key already existing: "+name_);
  return;
}
